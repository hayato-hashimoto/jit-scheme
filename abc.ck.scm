...
(load "./free-vars.scm")
(define (compile-lambda-p3 s base-addr)
;  args
;
;  body
;    (builtin expr)
...
;
  ...
  (define stack-variable-count 0)
  (define stack-variable-table '())
  (define heap-variables (allocate-vars body))
  (define code '())
  (define procs '())
  (define (make-variable name)
    (if (any (cut eq? <> name) allocate-vars)
      `(heap-variable ,heap-variable-count)   (set! heap-variable-count (+ 1 heap-variable-count)))
      `(stack-variable ,stack-variable-count) (set! stack-variable-count (+ 1 stack-variable-count)))
;  arguments
  (for-each (lambda (a)
    (push! stack-variable-table (cons a (make-variable #f)))) args)
  (push! code `(arguments ,(length args)))
  (make-variable #f) ; room for the return address
  (if (not (null? heap-variables)) (make-variable #f)) ; room for the heap pointer
  (push! (code) (allocate (length heap-variables )))
;  body
  (let loop ((sexp body))
  (define body1-addr #f)
  (define body2-addr #f)
  ...
  (cond
    ...
    ((?procedure? sexp)
      (set! procs (append (compile-lambda-p3 sexp base-addr) procs))
      (let1 p (car procs)
        (push! code `((builtin const64 ,const64) ,(relocation-symbol (lambda (i) 0) (lambda (i) (+ base-addr (car p))))))))
    ((?symbol? sexp)    (push! code 
      (if (assq sexp frame-stack-table) `(,b-lref ,(cdr (assq sexp stack-variable-table))) `((builtin refrdx ,ropmov64)))))
    ...
    ((and (?builtin? (car sexp)) (null? (cdddr sexp)) (symbol? (cadr sexp))) (loop (caddr sexp)) (push! code `(,(car sexp) ,(cdr (assq (cadr sexp) stack-variable-table)))))
    ((and (?builtin? (car sexp)) (null? (cdddr sexp))) (loop (caddr sexp)) (let1 y (make-stack-variable) (push! code `(push ,y)) (loop (cadr sexp)) (push! code `(,(car sexp) ,y))))
    ...
    ((?procedure? (car sexp))
      (set! procs (append (compile-lambda-p3 (car sexp) base-addr) procs))
      (let1 p (car procs)
       (for-each (lambda (e) (loop e) (push! code `(push ,(make-stack-variable)))) (cdr sexp))
  ...
  (for-each (lambda (s)
    (cond
      ((eq? (car s) 'arguments) (set! depth (+ depth (* 8 (cadr s)) 8)) (set! ret (* 8 (cadr s)))
           (set! code (append `((,b-frame ,(- depth ret)) (,b-call) (,b-frame ,(- ret depth)) ((builtin refrdx ,ropmov64))) code)))
      ...
      ((eq? (car s) 'call)      (set! code (append `((,b-frame ,(- depth ret)) (,b-call) (,b-frame ,(- ret depth))) code)))
...
(define (binary-compile s base-addr)
  (let1 procs (compile-lambda-p3 s base-addr)
    (size procs)
    (with-output-to-string 
      (lambda ()
        (dump-procs procs)))))

(define (opcode-compile s base-addr)
  (opcode-print (compile-lambda-p3 s base-addr)))
...
(define (compile s)
  (cond
    ...
    ((eq? s 'values) `(builtin values ,values2))
    ((eq? s 'if) `(syntax if))
    ((not (pair? s)) s)
    (else (map compile s))))
